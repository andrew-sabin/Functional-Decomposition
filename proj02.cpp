  ...
  setMonth( );
	...
	//. . .
        ...
        fprintf(stderr, "%2.2f, %2.2f, %2.2f, %2d, %2d\n",
                NowTemp, NowPrecip, NowHeight, NowNumDeer, NowNumWarios);
#else
        fprintf(stderr, "temp: %2.2f; precip: %2.2f ; height: %2.2f ; deer: %2d; warios: %2d \n",
                NowTemp, NowPrecip, NowHeight, NowNumDeer, NowNumWarios);
#endif 
  }
...
void Wario()
{
  ...
  // compute a temporary next-value for this quantity
	// based on the current state of the simulation:
	//. . .
  int NextNumWarios = NowNumWarios;
  int WcarryingCapacity = NowNumDeer + 1;

	// DoneComputing barrier:
	WaitBarrier( );
	//. . .
  if (NextNumWarios < WcarryingCapacity){
    NextNumWarios++;
  }
  else{
    NextNumWarios--;
